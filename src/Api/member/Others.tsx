...
function readProfile(otherUserId: number) {
  return GetAPI<{ data: OtherUserData[] } & statusResponse>(`/profile?targetUserId`, {
    otherUserId,
  });
}
...
function bringBoardList(otherUserId: number) {
  return GetAPI<{ data: OWriteBoardList } & statusResponse>(
    `/profile/board?page=1&recordSize=100000&targetUserId`,
    { otherUserId },
  );
}
...
//좋아요한 글 목록 가져오기
function likeBoardList() {
  return GetAPI<{ data: OLikeContentList } & statusResponse>(`/profile/like?page=1&recordSize=10`);
}
